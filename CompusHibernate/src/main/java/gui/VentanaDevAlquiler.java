...
public class VentanaDevAlquiler extends javax.swing.JFrame {
    ...
    public VentanaDevAlquiler(UserN usern, SessionFactory sf) {
        setLookAndFeel();
        initComponents();
        this.sf = sf;
        this.usern = usern;
    ...
    private void devLblMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_devLblMouseClicked
        ...
        if(filas > 0){
            ...
            for(int i = 0; i < filas; i++){
                ...
                disp.setIdDisp(Integer.parseInt((String)selecTab.getValueAt(i, 1)));

                comp = new Computadora();
                comp.setDisp(disp);

                alquiN = new Alquiler();
                alquiN.setAlquilador(usern);
                alquiN.setCompu(comp);
                alquiN.setFechIniAlqui(Date.valueOf((String)selecTab.getValueAt(i,2)));
                alquiN.setFechFinAlqui(Date.valueOf((String)selecTab.getValueAt(i,3)));
                alquiN.setFechDevAlqui(fechAct);

                alquiN.setIdAlqui((String)selecTab.getValueAt(i,0));
                
                alquiV = new Alquiler();
                alquiV.setAlquilador(usern);
                alquiV.setCompu(comp);
                alquiV.setFechIniAlqui(Date.valueOf((String)selecTab.getValueAt(i,2)));
                alquiV.setFechFinAlqui(Date.valueOf((String)selecTab.getValueAt(i,3)));

                alquiV.setIdAlqui((String)selecTab.getValueAt(i,0));
                
                bit = new Bitacora();
                bit.setAccbit("UPDATE");
                bit.setDatoNBit(alquiN.toString());
                bit.setDatoVBit(alquiV.toString());
    ...
    private ArrayList<Object[]> getCompusAlq(){
        ArrayList<Object[]> alqs;
        
        alqs = Ejecutor.getAlqNoPrest(sf, usern);
    ...
    private void setCompusAlq(){
        ...
        for(Object[] o : alqs){
            fila    = new Object[4];
            fila[0] = o[0];
            fila[1] = String.valueOf(o[1]);
            fila[2] = String.valueOf(o[2]);
            fila[3] = String.valueOf(o[3]);
            modelo.addRow(fila);
        }
    ...
    private void setFechAct(){
        ...
        cq = cb.createQuery(Date.class);
        
        cq.select(cb.currentDate());
        
        tq = s.createQuery(cq);
        
        try{
            fechActTxtBox.setText(String.valueOf(tq.getSingleResult()));
        }catch(NoResultException ex){
            
            System.out.println("ERROR AL OBTENER LA FECHA ACTUAL");
            s.close();
        }
