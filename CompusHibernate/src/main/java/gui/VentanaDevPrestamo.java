...
public class VentanaDevPrestamo extends javax.swing.JFrame {
    ...
    public VentanaDevPrestamo(UserN usern, SessionFactory sf) {
        setLookAndFeel();
        initComponents();
        this.usern = usern;
        this.sf = sf;
    ...
    private void devLblMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_devLblMouseClicked
        ...
        int filas;
        int[] idComp, idDoc;
        String[] idPrest, fechIniPrest, fechFinPrest;
        ...
        if(filas > 0){
            ...
            idPrest = new String[filas];
            idComp = new int[filas];
            idDoc = new int[filas];
            fechIniPrest = new String[filas];
            fechFinPrest = new String[filas];
   
            for(int i = 0; i < filas; i++){
                idPrest[i] = (String)selecTab.getValueAt(i, 0);
                idComp[i] = Integer.parseInt((String)selecTab.getValueAt(i, 1));
                idDoc[i] = (Integer)selecTab.getValueAt(i, 2);
                fechIniPrest[i] = (String)selecTab.getValueAt(i,3);
                fechFinPrest[i] = (String)selecTab.getValueAt(i,4);
            }
            
            ventana = new VentanaEstDispDev(usern, sf, idPrest, idComp, idDoc, fechIniPrest, fechFinPrest, fechAct, this);
    ...
    private ArrayList<Prestamo> getPrest(){
        ArrayList<Prestamo> prest;
        
        prest = Ejecutor.getPrest(sf, usern);
    ...
    private void setCompusPrest(){
        ...
        for(Prestamo p : prest){
            fila    = new Object[5];
            fila[0] = p.getIdPrest();
            fila[1] = p.getCompu().getDisp().getIdDisp() + "";
            fila[2] = p.getPresta().getIdUsrN();
            fila[3] = p.getFechIniPrest() + "";
            fila[4] = p.getFechFinPrest() + "";
            modelo.addRow(fila);
        }
