...
import compush.compushibernate.seguridad.Sesion;
...
//        botonDesb.setBackground(new Color(203, 182, 184));
    ...
    private void botonLoginMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_botonLoginMouseClicked
        ...
        q = s.createQuery(c.select(usr)
                .where(
                    cb.and(
                            cb.equal(usr.get("nomUsrN"), cb.parameter(String.class,"nomUsr")),
                            cb.equal(usr.get("passUsrN"), cb.parameter(String.class, "pass")))))
                
                .setParameter("nomUsr", userTextBox.getText()).setParameter("pass",String.valueOf(contTextBox.getPassword()));
        
        try{
            usern = (UserN) q.getSingleResult();
            sesion = new Sesion();
            uuid = UUID.randomUUID();
            
            sesion.setIdSesion(uuid.toString());
            sesion.setPidSesion(123);
            sesion.setUserN(usern);
            sesion.setActSesion(true);
            
            s.save(sesion);
            
            ventana = new VentanaIU(usern, sf);
